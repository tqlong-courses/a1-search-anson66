...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        cornerList = (self.corners[0], self.corners[1],
        ...
        for j in range(foodGrid.width):
            ...
            if foodGrid[j][i]:
                distance.append(mazeDistance(
                    (j, i), position, problem.startingGameState))
    if len(distance) == 0:
        return 0
    return max(distance)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # print(point)
        # print(tuple(point))
        prob = PositionSearchProblem(gameState, start=startPosition, goal=tuple(
            point), warn=False, visualize=False)
        # print("Hello")
        # print(prob)
